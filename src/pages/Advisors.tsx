...
const Advisors = () => {
  ...
  const handleEditAdvisor = (advisor: SimplifiedAdvisor) => {
    ...
    const fullAdvisor: Advisor = {
      ...advisor,
      UserCreation: selectedAdvisor?.UserCreation || 0,
      CreatedAt: selectedAdvisor?.CreatedAt || new Date(),
      UserUpdate: selectedAdvisor?.UserUpdate || 0,
      UpdatedAt: selectedAdvisor?.UpdatedAt || new Date(),
      Active: selectedAdvisor?.Active || true,
    };
  ...
  const handleSaveChanges = () => {
    const newErrors = new AdvisorErrors(
      !newAdvisor.Enrollment ? "Campo requerido" : "",
      !newAdvisor.Gender ? "Campo requerido" : "",
      !newAdvisor.Name ? "Campo requerido" : "",
      !newAdvisor.DegreeIdentity ? "Campo requerido" : ""
    );
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      if (selectedAdvisor) {
        const updatedAdvisors = advisors.map((advisor) =>
          advisor.Enrollment === selectedAdvisor.Enrollment
            ? newAdvisor
            : advisor
        );
  ...
  const filteredAdvisors: SimplifiedAdvisor[] = advisors
    .filter((advisor) => {
      ...
      return (
        ...
        (!searchStudentId ||
          regexStudentId.test(advisor.Enrollment.toString())) &&
  ...
  useEffect(() => {
    const fetchAdvisors = async () => {
      ...
      try {
        const data = await getAllAdvisorsDummy();
  ...
  return (
    <Container className="mt-4 bg-white" style={{ minHeight: "100vh" }}>
      ...
      <ClockInModal
        ...
        handleClose={handleCloseClockInModal}
      />
      <AdvisorModal
        show={showAdvisorModal}
        handleClose={handleCloseAdvisorModal}
        handleSaveChanges={handleSaveChanges}
        advisor={newAdvisor}
        handleInputChange={handleInputChange}
        errors={errors}
        mode={selectedAdvisor ? "Editar" : "Agregar"}
      />
    </Container>
  );
};
