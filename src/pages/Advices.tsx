...
const Advices = () => {
  ...
  const [selectedAdvice, setSelectedAdvice] = useState<AdvisorySession | null>(
    null
  );
  ...
  const handleSaveChanges = () => {
    ...
    const newErrors = {
      ...
      LearningUnitIdentity: newAdvice.LearningUnitIdentity
        ? ""
        : "Campo requerido",
    ...
    if (Object.values(newErrors).every((error) => error === "")) {
      if (selectedAdvice) {
        const updatedAdvices = advices.map((advice) =>
          advice.AdviseeStudentId === selectedAdvice.AdviseeStudentId
            ? newAdvice
            : advice
        );
